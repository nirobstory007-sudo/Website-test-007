...
router.post('/verify', limiter, requireApiKey('verify'), (req, res) => {
  ...
  if (!key || !device_id) { log('invalid'); return res.status(400).json({ valid: false, reason: 'missing_input' }); }
  ...
  if (row.banned) { log('banned', row.id); return res.json({ valid: false, reason: 'banned', message: 'This license has been banned.' }); }
  if (row.status === 'revoked') { log('revoked', row.id); return res.json({ valid: false, reason: 'revoked' }); }

  const start = row.used_at || row.created_at;
  const expiresAt = start + row.duration_days * 86400;
  if (expiresAt < now) { log('expired', row.id); return res.json({ valid: false, reason: 'expired', expires_at: expiresAt }); }
  ...
  if (!existing) {
    ...
    if (count >= maxDevices) { log('device_limit', row.id); return res.json({ valid: false, reason: 'device_limit', message: `Maximum of ${maxDevices} device(s) reached.` }); }
    db.prepare('INSERT INTO key_devices (key_id, device_id) VALUES (?,?)').run(row.id, device_id);
  } else {
    db.prepare(`UPDATE key_devices SET last_seen=strftime('%s','now') WHERE key_id=? AND device_id=?`).run(row.id, device_id);
  }
  if (!row.used_at) db.prepare(`UPDATE keys SET device_id=?, status='used', used_at=? WHERE id=?`).run(device_id, now, row.id);
...
router.get('/check', requireApiKey('verify'), (req, res) => {
  ...
  const row = db.prepare(`SELECT id,banned,status,device_tier,duration_days,created_at,used_at FROM keys WHERE key_value=?`).get(key);
  ...
  const expiresAt = (row.used_at || row.created_at) + row.duration_days * 86400;
  if (expiresAt < Math.floor(Date.now() / 1000)) return res.json({ valid: false, reason: 'expired' });
    ...
    db.transaction(() => {
      ...
      for (let i = 0; i < countInt; i++) {
        for (let a = 0; a < 5; a++) {
          ...
          try {
            const info = db.prepare(`INSERT INTO keys (key_value,prefix,duration_days,device_tier,created_by,owner_id) VALUES (?,?,?,?,?,?)`).run(kv, prefix, daysInt, tier, me.id, me.id);
            created.push(kv);
            break;
          } catch {}
        }
