...
async function renderOverview(el) {
  ...
  try {
    const res = await fetchT('/api/keys?per_page=200', {}, 20000);
    if (res.ok) {
    ...
    console.error('keys fetch failed:', e);
    loadError = true;
  }
  ...
  if (loadError) {
    el.innerHTML = `
      <div class="card">
        <h2>⏳ Server is slow or unreachable</h2>
        <p class="muted">This usually means the free Render instance is waking up.</p>
        <button class="primary" style="margin-top:12px" onclick="location.reload()">🔄 Retry</button>
      </div>`;
    return;
  }
