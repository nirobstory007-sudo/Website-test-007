...
async function boot() {
  try {
    ...
    document.getElementById('main').innerHTML =
      '<div class="card"><h2>Connection error</h2><p class="muted">Could not reach the server. Please refresh.</p></div>';
  }
}
...
async function showView(name) {
  ...
  try {
    ...
    main.innerHTML = `<div class="card">
      <h2>⚠️ Error loading view: ${name}</h2>
      <p class="muted">${e.message || 'Unknown error'}</p>
      <p class="muted">Refresh the page and try again.</p>
    </div>`;
  }
}
...
async function renderOverview(el) {
  let keys = [];
  try {
    const res = await fetch('/api/keys?per_page=200');
    if (res.ok) {
      const data = await res.json();
      keys = data.keys || [];
    } else {
      console.error('keys fetch failed:', res.status);
    }
  } catch (e) {
...
async function renderUsers(el) {
  ...
  /* Build role options based on current user's rank */
  const roleOptions = [];
...
async function renderMaster(el) {
  let prefix = 'DEMO';
  try {
    const { prefix: p } = await fetch('/api/keys?per_page=1').then(r => r.json());
    if (p) prefix = p;
  } catch {}
